...
function Write-BaseThemeFile {
  param([string]$ProjectDir)

  $uiDir = Join-Path $ProjectDir "src/ui"
...
'@
  Write-Utf8NoBom -Path (Join-Path $uiDir "theme.ts") -Content $themeContent
}

function Write-BaseAppFiles {
  param([string]$ProjectDir)

  Write-BaseThemeFile -ProjectDir $ProjectDir
...
export default function HomeScreen() {
  ...
  return (
    <View style={{ flex: 1, justifyContent: "center", alignItems: "center", padding: 24, backgroundColor: colors.background }}>
      <Text style={{ fontSize: 28, fontWeight: "700", color: colors.text }}>Welcome</Text>
...
export default function ExploreScreen() {
  ...
  return (
    <View style={{ flex: 1, justifyContent: "center", alignItems: "center", padding: 24, backgroundColor: colors.background }}>
      <Text style={{ fontSize: 24, fontWeight: "700", color: colors.text }}>Explore</Text>
      <Text style={{ marginTop: 10, color: colors.textMuted }}>Second-route baseline for smoke checks.</Text>
    </View>
  );
}
'@
...
function Configure-AppJson {
  ...
  Set-ObjectProperty -Object $expo -Name "userInterfaceStyle" -Value "automatic"
